...
class rectangle 
{
    /* h: height | b: base | l: Length | p: density
    mP: mass payload | alphaMax: max angular acc | yield: yield strength |
    stepRatio: ratio of increment or decrement in optimization
    */
    private:
    double h, b, l, p, mP, alphaMax, yield, stepRatio, safetyFactor;
    string crossSectionShape,materialName;
    public:
    rectangle(double h =0, double b =0,double l=0, double p =0, double mP=0, double alphaMax=0, double yield =0, double stepRatio=1, double safetyFactor = 100)
    {
        this-> h=h;
        this-> b=b;
        this-> l=l;
        ...
        void handleRectangle(const Material& selected, link& T1)
    {
        ...
        Treq = torqueRec(MassRec(), l/1000, mP, alphaMax);
...
class circle 
{
    /* r: radius | l: Length | p: density
    mP: mass payload | alphaMax: max angular acc | yield: yield strength |
    stepRatio: ratio of increment or decrement in optimization
    */
    private:
    double r, l, p, mP, alphaMax, yield, stepRatio, safetyFactor; //justify why safety factor is double not float
    string crossSectionShape,materialName;
    public:
    circle(double r =0, double l=0, double p =0, double mP=0, double alphaMax=0, double yield =0, double stepRatio=1, double safetyFactor = 100)
    {
        this-> r=r;
        this-> l=l;
        this-> p=p;
    ...
    void handleCircle(const Material& selected, link& C1)
    {
        ...
        mP = ValidDouble("[+] Payload (kg): ");
        alphaMax = ValidDouble("[+] Max angular acceleration (rad/s^2): ");
        Treq = torqueRec(MassCirc(), l/1000, mP, alphaMax);
        Wreq = ValidDouble("[+] What is your required speed? [rpm]: ")*2*M_PI/60;
        stepRatio = ValidDouble("[+] Step ratio (%) [default 1%]: ");
        safetyFactor = ValidDouble("[+] Safety factor (%) [default 100% from yield]: ");
        flow_func_circ();
...
class link // Class to represent a link then passing to suitable cross section
{
    private:
    ...
    void handlingLinkShape(Material selected, link &L1)
    {
        ...
        {
            C1.handleCircle(selected, L1);
        }
        else if (this->crossSectionShape == "rectangle")
        {
            R1.handleRectangle(selected, L1);
        }
    }
